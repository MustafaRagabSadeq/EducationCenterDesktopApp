...
namespace trainingCenter
{
    ...
    public partial class addOutcome : MetroSetForm
    {
            ...
            try
            {
                if (txt4.Text.Length > 0)
                {
                    ...
                    if (Isvalid)
                    {
                        if (money > 0)
                        {
                            if (money <= Convert.ToDouble(txt3.Text) && money > 0)
                            {
                                ...
                                {
                                    Person_ID = stid,
                                    Name = $"سحب الطالب {student.St_Name} من رصيده",
                                    Price = money,
        ...
        private void addOutcome_FormClosed(object sender, FormClosedEventArgs e)
        {
            context.Dispose();
        }
    } 
