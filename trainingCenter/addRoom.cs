            ...
            try
            {
                ...
                if (id > 0)
                {
                    if (textBox1.Text.Length > 0 && textBox2.Text.Length > 0 && txtCode.Text.Length > 0)
                    {
                        ...
                        DialogResult dialogResult = MessageBox.Show("هل أنت متأكد من الحذف", "حذف قاعة", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
                        if (dialogResult == DialogResult.OK)
                        {
                            Schedule schedule = context.Schedules.Where(x => x.Room_ID == id).FirstOrDefault();
                            if (schedule == null)
                            {
                            ...
                            {
                                MessageBox.Show("لا يمكنك حذف القاعة ! برجاء حذف المواعيد المسجلة للقاعة في جدول المواعيد ", "حذف قاعة", MessageBoxButtons.OK, MessageBoxIcon.Error);
